...
struct bPoseChannel_Runtime { 
	...
	bPoseChannel_BBoneSegmentBoundary *bbone_segment_boundaries;
	void *_pad;
}; 
...
struct bAction { 
	ID id;
	ActionLayer **layer_array;
	int layer_array_num;
	int layer_active_index;
	ActionSlot **slot_array;
	int slot_array_num;
	int last_slot_handle;
	ActionStripKeyframeData **strip_keyframe_data_array;
...
struct ActionStripKeyframeData { 
	ActionChannelBag **channelbag_array;
...
struct BoneCollectionMember { 
	BoneCollectionMember *next;
	BoneCollectionMember *prev;
	Bone *bone;
}; 

struct BoneCollectionReference { 
	BoneCollectionReference *next;
	BoneCollectionReference *prev;
...
struct AssetWeakReference { 
	...
	char *asset_library_identifier;
	char *relative_asset_identifier;
}; 

struct AssetCatalogPathLink { 
	AssetCatalogPathLink *next;
	AssetCatalogPathLink *prev;
	char *path;
}; 

struct AttributeArray { 
	void *data;
	ImplicitSharingInfoHandle *sharing_info;
	int64_t size;
}; 
...
struct BrushCurvesSculptSettings { 
	...
	uchar density_mode;
	char _pad[7];
	CurveMapping *curve_parameter_falloff;
}; 
...
struct CollectionExport { 
	CollectionExport *next;
	CollectionExport *prev;
	char fh_idname[64];
	char name[64];
...
struct GreasePencilLayerMask { 
	GreasePencilLayerMask *next;
	GreasePencilLayerMask *prev;
	char *layer_name;
	ushort flag;
	char _pad[6];
}; 
...
struct GreasePencilLayerTreeNode { 
	GreasePencilLayerTreeNode *next;
	GreasePencilLayerTreeNode *prev;
...
struct LightProbeObjectCache { 
	...
	char dirty;
	char _pad0[2];
	LightProbeGridCacheFrame *grid_static_cache;
}; 
...
struct SurfaceModifierData_Runtime { 
	float (*vert_positions_prev)();
	float (*vert_velocities)();
	Mesh *mesh;
...
struct MeshDeformModifierData { 
	...
	ImplicitSharingInfoHandle *bindinfluences_sharing_info;
	int *bindoffsets;
	ImplicitSharingInfoHandle *bindoffsets_sharing_info;
	float *bindcagecos;
	ImplicitSharingInfoHandle *bindcagecos_sharing_info;
	int totvert;
	int totcagevert;
	MDefCell *dyngrid;
	ImplicitSharingInfoHandle *dyngrid_sharing_info;
	MDefInfluence *dyninfluences;
	ImplicitSharingInfoHandle *dyninfluences_sharing_info;
	int *dynverts;
	ImplicitSharingInfoHandle *dynverts_sharing_info;
	int dyngridsize;
	int totinfluence;
	float dyncellmin[3];
	float dyncellwidth;
...
struct CorrectiveSmoothModifierData { 
	ModifierData modifier;
	float (*bind_coords)();
	ImplicitSharingInfoHandle *bind_coords_sharing_info;
	int bind_coords_num;
	float lambda;
	float scale;
	short repeat;
...
struct LaplacianDeformModifierData { 
	...
	int repeat;
	float *vertexco;
	ImplicitSharingInfoHandle *vertexco_sharing_info;
	void *cache_system;
	short flag;
	char _pad[6];
}; 
...
struct SurfaceDeformModifierData { 
	...
	Object *target;
	SDefVert *verts;
	ImplicitSharingInfoHandle *verts_sharing_info;
	float falloff;
	int num_mesh_verts;
	int numverts;
...
struct GreasePencilLineartModifierData { 
	...
	LineartCache *cache;
	LineartData *la_data_ptr;
	LineartModifierRuntime *runtime;
}; 
...
struct bNodeSocketValueMenu { 
	int value;
	int runtime_flag;
	RuntimeNodeEnumItemsHandle *enum_items;
}; 
...
struct RigidBodyWorld_Shared { 
	PointCache *pointcache;
	ListBase ptcaches;
	RigidBodyWorld_Runtime *runtime;
}; 
...
struct NamedBrushAssetReference { 
	NamedBrushAssetReference *next;
	NamedBrushAssetReference *prev;
	char *name;
...
struct Paint { 
	...
	AssetWeakReference *brush_asset_reference;
	Brush *eraser_brush;
	AssetWeakReference *eraser_brush_asset_reference;
...
struct Sculpt { 
	...
	float automasking_view_normal_falloff;
	CurveMapping *automasking_cavity_curve;
	CurveMapping *automasking_cavity_curve_op;
...
struct ToolSettings { 
	...
	short snap_step_seconds;
	short snap_step_frames;
	int playhead_snap_distance;
	Object *anim_mirror_object;
	Object *anim_relative_object;
	char anim_mirror_bone[64];
	uchar fix_to_cam_flag;
	char _pad8[7];
}; 
...
struct Scene { 
	...
	char _pad3[1];
	bNodeTree *nodetree;
	bNodeTree *compositing_node_group;
	Editing *ed;
...
struct LayoutPanelState { 
	LayoutPanelState *next;
	LayoutPanelState *prev;
	char *idname;
	uchar flag;
	char _pad[3];
	int last_used;
...
struct uiViewStateLink { 
	uiViewStateLink *next;
	uiViewStateLink *prev;
	char idname[64];
	uiViewState state;
}; 
...
struct EQCurveMappingData { 
	EQCurveMappingData *next;
	EQCurveMappingData *prev;
	CurveMapping curve_mapping;
}; 
...
struct FileSelectParams { 
	...
	short list_thumbnail_size;
	short list_column_size;
	short type;
	short flag;
	short sort;
	short display;
...
struct FileFolderHistory { 
	FileFolderLists *next;
	FileFolderLists *prev;
	char browse_mode;
	char _pad[7];
...
struct ThemeCommonAnim { 
	char playhead[4];
	char preview_range[4];
	char channels[4];
	char channels_sub[4];
	char channel_group[4];
	char channel_group_active[4];
	char channel[4];
	char channel_selected[4];
	char keyframe[4];
	char keyframe_extreme[4];
	char keyframe_breakdown[4];
	char keyframe_jitter[4];
	char keyframe_moving_hold[4];
	char keyframe_generated[4];
	char keyframe_selected[4];
	char keyframe_extreme_selected[4];
	char keyframe_breakdown_selected[4];
	char keyframe_jitter_selected[4];
	char keyframe_moving_hold_selected[4];
	char keyframe_generated_selected[4];
	char long_key[4];
	char long_key_selected[4];
	char scene_strip_range[4];
	char _pad0[4];
}; 
...
struct ThemeUI { 
	...
	char editor_border[4];
	char editor_outline[4];
	char editor_outline_active[4];
	...
	char panel_header[4];
	char panel_back[4];
	char panel_sub_back[4];
	char panel_outline[4];
	char panel_title[4];
	char panel_text[4];
	char panel_active[4];
}; 
...
struct ThemeSpace { 
	...
	char edge[4];
	char edge_select[4];
	char edge_mode_select[4];
	char face[4];
	char face_select[4];
	char face_mode_select[4];
	char face_retopology[4];
	char face_back[4];
	char face_front[4];
	...
	char strip[4];
	char strip_select[4];
	char before_current_frame[4];
	char after_current_frame[4];
	...
	char node_zone_simulation[4];
	char node_zone_repeat[4];
	char node_zone_foreach_geometry_element[4];
	char node_zone_closure[4];
	char simulated_frames[4];
	char movie[4];
	char movieclip[4];
	char mask[4];
	char image[4];
	...
	char text_strip_cursor[4];
	char selected_text[4];
...
struct bUserExtensionRepo { 
	bUserExtensionRepo *next;
	bUserExtensionRepo *prev;
	char name[64];
	char module[48];
	char *access_token;
...
struct bUserScriptDirectory { 
	bUserScriptDirectory *next;
	bUserScriptDirectory *prev;
	char name[64];
	char dir_path[768];
}; 

struct bUserAssetShelfSettings { 
	bUserAssetShelfSettings *next;
	bUserAssetShelfSettings *prev;
...
struct EvaluateClosureNodeViewerPathElem { 
	ViewerPathElem base;
	int evaluate_node_id;
	int source_output_node_id;
	bNodeTree *source_node_tree;
}; 
